...
class Module:
    def __init__(self, imports: Iterable[Tuple[str, str]], statements: Iterable[asttypes.Node], exports):
        self.imports = imports
        self.statements = statements
        self.exports = exports

    def formatted_imports(self) -> Iterable[str]:
        if not self.imports:
            return
    ...
    def formatted_statements(self) -> Iterable[str]:
        if not self.statements:
            return
        yield from ([str(statement) for statement in self.statements])

    def formatted_exports(s) -> Iterable[str]:
        if not s.exports:
            return
...
def formatted_module(tree: asttypes.ES5Program) -> Iterable[str]:
    return flatten(m.formatted() for m in module(tree))
